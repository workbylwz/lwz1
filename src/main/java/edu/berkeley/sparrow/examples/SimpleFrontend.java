...
public class SimpleFrontend implements FrontendService.Iface {
  ...
  public static final int DEFAULT_EXPERIMENT_S = 800;

  public static final String JOB_ARRIVAL_PERIOD_MILLIS = "job_arrival_period_millis";
  public static final int DEFAULT_JOB_ARRIVAL_PERIOD_MILLIS = 50;
  ...
  private SparrowFrontendClient client1;
  private int count = 0;
  ...
  private class JobLaunchRunnable implements Runnable {
	...
	if (count%2 == 0){
		...
		client1.submitJob(APPLICATION_ID, tasks, USER);
	}
	count += 1;
      } catch (TException e) {
  ...
  public void run(String[] args) {
    try {
      ...
      String schedulerHost1 = "172.17.0.3";
      client = new SparrowFrontendClient();
      client1 = new SparrowFrontendClient();
      client.initialize(new InetSocketAddress(schedulerHost, schedulerPort), APPLICATION_ID, this);
      client1.initialize(new InetSocketAddress(schedulerHost1, schedulerPort), APPLICATION_ID, this);
